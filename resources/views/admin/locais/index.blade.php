            ...
            <div class="card-header bg-dark d-flex align-items-center justify-content-between">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    @if(Auth::user()->isAdminSistema())
                        <a href="{{ route('admin.locais.create') }}" class="btn btn-outline-info btn-sm d-flex align-items-center">
                            ...
                            <div class="card mb-3 border border-light-subtle shadow-sm rounded">
                                <div class="card-body p-3">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <h5 class="card-title fw-bold mb-0 text-primary">{{ $local->adm_local }}</h5>
                                        <span class="badge bg-light-info text-info">ID: {{ $local->admlc_id }}</span>
                                    </div>
                                    
                                    <div class="mb-3 small text-dark">
                                        <div class="mb-1"><strong>CNPJ:</strong> <code>{{ $local->cnpj }}</code></div>
                                        <div class="mb-1"><strong>Cidade/UF:</strong> {{ $local->cidade }}/{{ $local->uf }}</div>
                                        <div class="mb-1"><strong>Regional:</strong> {{ $local->regional->adm_regional ?? 'N/A' }}</div>
                                        <div class="mt-2">
                                            <span class="badge bg-light-primary text-primary fw-bold me-1" title="Igrejas">
                                                {{ $local->igrejas_count }} Igrejas <i class="ti ti-building-community"></i>
                                            </span>
                                            @if($local->setores_count > 0)
                                                <span class="badge bg-light-primary text-primary fw-bold" title="Setores">
                                                   {{ $local->setores_count }} Setores <i class="ti ti-chart-bar"></i>
                                                </span>
                                            @endif
                                        </div>
                                    </div>
