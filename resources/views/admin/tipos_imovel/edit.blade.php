...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 animate-fadeIn max-w-2xl mx-auto">
        <h2 class="text-lg font-bold text-gray-800 mb-6">Editar Tipo de Imóvel</h2>
        ...
        <form action="{{ route('tipos-imovel.update', $tiposImovel->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="nome" class="block text-xs font-bold text-gray-500 uppercase mb-2">Nome do Tipo</label>
                <input type="text" name="nome" id="nome" value="{{ old('nome', $tiposImovel->nome) }}"
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                <p class="text-xs text-blue-700">
                    <strong>Atenção:</strong> Este tipo está sendo usado por {{ $tiposImovel->igrejas()->count() }}
                    localidade(s).
                    Alterar o nome afetará todas elas.
                </p>
            </div>
